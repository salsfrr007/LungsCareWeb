...
<head>
  ...
  <title>Add Article - LungsCare Admin</title>
...
<div class="flex min-h-screen">
  ...
  <main class="flex-1 p-10">
    <h2 class="text-2xl text-white font-bold mb-6">Tambah Artikel</h2>
    ...
    <form action="{{ route('articles.store') }}" method="POST" class="bg-white p-6 rounded shadow max-w-2xl">
      @csrf
      <div class="mb-4">
        <label for="judul" class="block font-semibold mb-1">Judul</label>
        <input type="text" id="judul" name="judul" value="{{ old('judul') }}" class="w-full border border-gray-300 rounded px-3 py-2" required />
      </div>

      <div class="mb-4">
        <label for="link" class="block font-semibold mb-1">Link</label>
        <input type="url" id="link" name="link" value="{{ old('link') }}" class="w-full border border-gray-300 rounded px-3 py-2" required />
      </div>

      <div class="mb-4">
        <label for="author" class="block font-semibold mb-1">Author</label>
        <input type="text" id="author" name="author" value="{{ old('author') }}" class="w-full border border-gray-300 rounded px-3 py-2" required />
      </div>

      {{-- <div class="mb-4">
        <label for="tanggal" class="block font-semibold mb-1">Tanggal</label>
        <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal') }}" class="w-full border border-gray-300 rounded px-3 py-2" required />
      </div> --}}

      <div class="mb-6">
        <label for="ringkasan" class="block font-semibold mb-1">Ringkasan</label>
        <textarea id="ringkasan" name="ringkasan" rows="4" class="w-full border border-gray-300 rounded px-3 py-2" required>{{ old('ringkasan') }}</textarea>
      </div>
