...
<head>
  ...
  <title>LungsCare Register Page</title>
...
<body class="white">
  <div class="min-h-screen flex flex-col md:flex-row">
    ...
    <!-- Left side (Slider) - Same as login -->
  ...
  <div class="md:w-1/2 w-full relative flex items-center justify-center bg-cover bg-no-repeat"
    ...
    <div class="relative z-10 w-full max-w-md px-6 py-10">
      <h1 class="text-white text-4xl font-extrabold mb-1">Create Account</h1>
      <p class="text-white text-xs mb-6">Join us and start your journey to a healthier life.</p>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus
                       class="w-full rounded-md border border-black border-opacity-40 px-4 py-2 text-black focus:outline-none focus:ring-2 focus:ring-[#8a9bf7] @error('name') border-red-500 @enderror">
            </div>
            ...
            <div class="text-center mt-4">
              <p class="text-sm text-white">
                  ...
                  <a href="{{ route('login') }}" class="font-medium text-white hover:underline">Login here</a>
              </p>
            </div>
        </form>
      </div>
  ...
  <script>
    ...
    function showSlide(index) {
      if (index === current || isTransitioning || !images[index]) return;
    ...
    if (images.length > 0 && img) {
        ...
        img.alt = "Welcome";
        if(caption) caption.textContent = "Create your LungsCare Account";
    }
  </script>
</body>
</html> 